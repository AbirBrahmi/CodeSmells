...
# Charger les données des occurrences de chaque type d'odeur de code par projet
file_path <- "C:/Users/Brahm/Documents/CodeSmells/RQ1/data/smell_count_per_project.csv"
...
# Calcul de la médiane pour chaque projet (occurrences des odeurs)
...
colnames(heatmap_data) <- c("Odeur", "Projet", "Occurrences")
colnames(heatmap_color_data) <- c("Odeur", "Projet", "Occurrences_Color")
...
# Création de la Heatmap avec Occurrences des Odeurs (avec tailles ajustées)
heatmap_plot <- ggplot(heatmap_combined, aes(x = Projet, y = Odeur, fill = Occurrences_Color)) +
  ...
  geom_text(aes(label = ifelse(!is.na(Occurrences), round(Occurrences, 0), "")), 
            color = "black", size = 3.6) +  # Texte ajusté à 3.6
  theme_minimal() +
  labs(title = "Occurrences de Chaque Type d'Odeur de Code par Projet",
       x = "Projets",
       y = "Odeurs de Code",
       fill = "Occurrences") +
