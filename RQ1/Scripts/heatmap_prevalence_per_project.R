...
file_path <- "C:/Users/Brahm/Documents/CodeSmells/RQ1/data/prevalence_by_project.csv"
...
# Calcul de la médiane pour chaque projet (prévalence)
...
colnames(heatmap_data) <- c("Odeur", "Projet", "Prévalence")
colnames(heatmap_color_data) <- c("Odeur", "Projet", "Prévalence_Color")
...
# Création de la Heatmap avec Prévalence (avec tailles ajustées)
heatmap_plot <- ggplot(heatmap_combined, aes(x = Projet, y = Odeur, fill = Prévalence_Color)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "white", high = "steelblue", na.value = "white", guide = guide_colorbar(position = "bottom")) +  # Échelle en bas
  geom_text(aes(label = ifelse(!is.na(Prévalence), round(Prévalence, 1), "")), 
            color = "black", size = 3.6) +  # Texte ajusté à 3.6
  theme_minimal() +
  labs(title = "Prévalence des Odeurs de Code par Projet (en %)",
       x = "Projets",
       y = "Odeurs de Code",
       fill = "Prévalence (%)") +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5, size = 11),  # Texte des noms des projets ajusté à 3.6
        axis.text.y = element_text(size = 11),  # Texte des noms des odeurs ajusté à 3.6
